...
hyperfine --warmup 20 --shell=none --export-markdown benchmarks-1.md --sort mean-time \
...
hyperfine --warmup 20 --shell=none --export-markdown benchmarks-2.md --sort mean-time \
...
Time for parsing pandoc-manual.dj ($(echo $(du -h pandoc-manual.dj) | sed -r 's/([0-9.MK]+)[ .a-zA-Z-]+/\1/')) into html:

$(cat benchmarks-1.md)

Time for parsing tartan-wikipedia.dj ($(echo $(du -h tartan-wikipedia.dj) | sed -r 's/([0-9.MK]+)[ .a-zA-Z-]+/\1/')) into html:
...
jotdown --version 2>> benchmarks.md
